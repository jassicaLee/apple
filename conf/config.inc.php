<?php
	...
	$pgadmin_host=@$_REQUEST['hostname'];
	if(isset($pgadmin_host)){
		setcookie("pgadmin_host", $pgadmin_host, 0);
	}else{
		$pgadmin_host=@$_COOKIE['pgadmin_host'];
	}
	$pgadmin_port=@$_REQUEST['port'];
	if(isset($pgadmin_port)){
		setcookie("pgadmin_port", $pgadmin_port, 0);
	}else{
		$pgadmin_port=@$_COOKIE['pgadmin_port'];
	}
	$pgadmin_user=@$_REQUEST['username'];
	if(isset($pgadmin_user)){
		setcookie("pgadmin_user", $pgadmin_user, 0);
	}else{
		$pgadmin_user=@$_COOKIE['pgadmin_user'];
	}
	$pgadmin_pass=@$_REQUEST['password'];
	if(isset($pgadmin_pass)){
		setcookie("pgadmin_pass", $pgadmin_pass, 0);
	}else{
		$pgadmin_pass=@$_COOKIE['pgadmin_pass'];
	}
	//echo $pgadmin_pass."=".$pgadmin_user.'----';
	//echo $_COOKIE['pgadmin_pass'].':'.$_COOKIE['pgadmin_user'];
	// Display name for the server on the login screen
	$conf['servers'][0]['desc'] = 'PostgreSQL';

	// Hostname or IP address for server.  Use '' for UNIX domain socket.
	// use 'localhost' for TCP/IP connection on this computer
	$conf['servers'][0]['host'] = $pgadmin_host;
	
	// Database port on server (5432 is the PostgreSQL default)
	$conf['servers'][0]['port'] = $pgadmin_port;
	// Database SSL mode
